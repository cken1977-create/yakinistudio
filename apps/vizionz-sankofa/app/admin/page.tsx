  ...
  return (
    <div>
      ...
      >
        ...
        <StatCell
          label="Google Sync"
          value={googleSyncCount ? 'Connected' : 'Not connected'}
          accent={googleSyncCount ? '#007A33' : 'rgba(10, 10, 10, 0.15)'}
        />
      </section>
...
type ActivityItem = {
  ...
  participants: { first_name: string; last_name: string } | { first_name: string; last_name: string }[] | null
}
  ...
  return (
    <section style={{ marginBottom: '32px' }}>
      ...
      <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {items.map((item) => {
          const p = Array.isArray(item.participants) ? item.participants[0] : item.participants
          ...
          return (
            ...
            >
              <div>
                ...
                <p style={{ fontSize: '13px', color: 'rgba(10, 10, 10, 0.6)', margin: 0, lineHeight: 1.5 }}>{preview}</p>
              </div>
              <span style={{ fontSize: '11px', color: 'rgba(10, 10, 10, 0.4)', whiteSpace: 'nowrap' }}>{date}</span>
            </li>
          )
        })}
      </ul>
